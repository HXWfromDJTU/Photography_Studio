	...
	<body>
		<div class="page page-settings">
			<header class="bar bar-nav">
				...
				<a onclick="$.editSet('add')" class="button button-link pull-right">
					<span class="icon icon-plus"></span> 新增套餐
				</a>
			</header>
			<div class="content">
		...
		<script>
			$.loadSetsMessageList = function(orderStatus) {
				$.ajax({
					type: "post",
					...
					success: function(d) {
						...
						if(data.code == "0000") {
							...
							data.data.forEach(function(arg, index) {
								tempStr.push('<li  id="set' + arg.id + '"  class="swipeout outter">');
								tempStr.push('<div onclick="$.editSet(\'' + arg.id + '\');"  class="swipeout-content">');
								...
								tempStr.push('<a onclick="$.deleteSet(\'' + arg.id + '\');" class="bg-danger swipeout" href="#">删除套餐</a>');
								tempStr.push('</div>');
								tempStr.push('</li>');
							});
							$('#userContainer').empty();
			...
			$.deleteSet = function(setID) {
				$.confirm("是否要删除这个套餐?", function() {
					$.ajax({
						type: "post",
						url: "<?php echo U('admin/deleteSet');?>",
						async: true,
						data: {
							setID: setID
						},
			...
			$.editSet = function(setID) {
					window.location.href = "http://localhost/hsylglxt/admin.php/index/editSets.html?setID=" + setID;
				}
